...
public class Database {
	...
	String passwd = "hong";		//본인이 설정한 root 계정의 비밀번호를 입력하면 된다.
	...
	boolean logincheck(String _i, String _p) {
		...
		try {
			String checkingStr = "SELECT pwd FROM haksamember WHERE id='" + id + "'";
			ResultSet result = stmt.executeQuery(checkingStr);
			
			int count = 0;
			while(result.next()) {
				if(pw.equals(result.getString("pwd"))) {
					flag = true;
	...
	boolean joinCheck(String _i, String _p) {
		...
		try {
			String insertStr = "INSERT INTO haksamember VALUES('" + id + "', '" + pw + "')";
